...
const NavItem: FunctionComponent<{activeItem: string, route: string, setActiveItem:Function, name:string}> = ({activeItem, route, setActiveItem, name}) => {
    return activeItem !== name ?(
         <Link to={route}> <a><span className="text-black hover:text-green" onClick={() => setActiveItem(name)}>{name}</span></a> </Link>
    ) : null
}
